...
- (void)viewDidLoad {
    ...
    self.arrData = [array sortedArrayUsingComparator:^NSComparisonResult(BaseVcashTxLog *obj1, BaseVcashTxLog *obj2) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row < self.arrData.count) {
        BaseVcashTxLog *txLog = self.arrData[indexPath.row];
        [cell setTxLog:txLog];
    }
    return cell;
}
